module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    var game = await Game.find({nome: inputs.nome});

    if(game.length > 0){//não cria diretorio se já existir um jogo com mesmo nome
      return exits.success({me: req.me, err: 'O nome do jogo já existe!'}); 
    }else{
      req.file('game').upload({
        ...
        .catch((err)=>{
          //return exits.success({me: req.me, err: err});
          return res.serverError(err);
        });
      });//.upload
    }//else
